...
let sheetRange = "A2:C200"; //SHEET_RANGE might have to be changed if we ever have >30 events, but probably not
let fullURL =
...
export default function Points() {
  ...
  // Fetch leaderboard data from Google Sheets on component mount
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        for (let i = 0; i < 10; i++) {
          var str =
            jsonData["table"]["rows"][i]["c"][1]["v"].toLowerCase() +
            ": " +
            jsonData["table"]["rows"][i]["c"][2]["f"] +
            " Points";
          board.push(str);
        }
